...
로컬 서버를 띄운 상태에서는 관리자 콘솔에서 pending 후보를 볼 수 있다.

```
http://127.0.0.1:8080/admin/events.html
```

현재 로그인은 로컬 검토용 임시 게이트이며 기본 PIN은 `kis-admin`이다. 실제 공개 사이트용 관리자 로그인은 백엔드/API 단계에서 세션과 권한 확인을 붙여야 한다.
